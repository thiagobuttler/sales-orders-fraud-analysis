...
class Pipeline:
    ...
    def run(self, config):
        ...
        pagamentos_pedidos_df = self.transformer.join_pagamentos_pedidos(
            pagamentos_df, pedidos_df
        )
        logger.info(
            f"Join entre pagamentos e pedidos realizado com sucesso! | Registros: {pagamentos_pedidos_df.count()}"
        )

        # Filtra apenas os pagamentos recusados, identificados como legítimo
        pagamentos_pedidos_df = self.transformer.filter_pagamentos_recusados(
            pagamentos_pedidos_df
        )
        logger.info(
            f"Filtro de pagamentos = false e fraude = false realizado com sucesso! | Registros: {pagamentos_pedidos_df.count()}"
        )

        # Apaga as colunas de status e fraude, desnecessárias para o df final
        pagamentos_pedidos_df = self.transformer.drop_status_fraude(
            pagamentos_pedidos_df
        )
        logger.info(
            f"Colunas status e fraude removidas com sucesso! | Registros: {pagamentos_pedidos_df.count()}"
        )

        # Filtra apenas os pedidos de 2025
        pagamentos_pedidos_df = self.transformer.filter_pedidos_2025(
            pagamentos_pedidos_df
        )
        logger.info(
            f"Filtro de pedidos de 2025 realizado com sucesso! | Registros: {pagamentos_pedidos_df.count()}"
        )

        # Ordena o df final pelas colunas de UF, forma de pagamento e data de criação do pedido
        pagamentos_pedidos_df = self.transformer.orderBy_pagamentos_pedidos(
            pagamentos_pedidos_df
        )
        logger.info(
            "Dataframe ordenado por UF, forma de pagamento e data de criação do pedido com sucesso!"
        )
